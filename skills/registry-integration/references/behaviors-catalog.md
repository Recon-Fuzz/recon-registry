...
> **Two valid shapes — both fine:**
> - **Package an existing faithful mock** (e.g. an ERC4626 tester): a single contract that takes its deps as **constructor args**, imports OpenZeppelin / the protocol's own code, and has **no** `rvm`/`asActor`/`asAdmin`/`fund()` — the operator auto-registers its functions as actions, drives them with actors, funds them, and registers the address under the entry name. This is the **common, minimal** case. Packaging it as-is IS the port.
> - **The fuller pattern** (the sketches below): a piece that **deploys its own internal deps** (loan/collateral tokens, a seeded pool). Those use `rvm` to `register` the internal deps under entry-name-prefixed labels and a re-callable `fund()` to mint/approve actors — and inline `asActor`/`asAdmin` only where shown. Use this only when the piece genuinely stands up its own sub-system.
>
> Pick the smallest shape that's faithful. Don't add `rvm`/modifiers/`fund()` to a mock that doesn't deploy its own deps.

> **Label rule (shared namespace):** the main contract resolves via `getAddr("<EntryName>")` (auto-registered — don't declare it). Any **internal** address a piece registers itself MUST be **prefixed with the entry name** — `rvm.register("MorphoMock.loanToken", addr)` — since the `getAddr` namespace is shared by the suite + all pieces, so bare `"market"`/`"pool"`/`"token"` collide. Declare ONLY those internal sublabels in `recon-registry.toml`; a self-contained mock → `labels = []`. (The sketches below name their contracts `LendingMock`/`AmmMock`/`OracleMock` and prefix their internal deps accordingly.)
